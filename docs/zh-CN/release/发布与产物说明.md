...
1. 确认根 `Cargo.toml`、`apps/package.json`、Tauri 的 `Cargo.toml` / `tauri.conf.json` 及两个 `Cargo.lock` 中的项目版本已同步
2. 确认 `CHANGELOG.md` 已更新
3. 确认桌面端前端构建通过：`pnpm -C apps run build:desktop`
4. 确认核心测试通过：`pnpm -C apps run test`、`cargo test --workspace`
5. 若涉及网关协议改动，执行 `cargo test -p codexmanager-service gateway --offline -- --test-threads=1`；仓库已移除旧的 `scripts/tests/gateway_regression_suite.ps1` 探针。
