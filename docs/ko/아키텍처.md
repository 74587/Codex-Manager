...
- `apps/src/components/layout/app-bootstrap.tsx`：前端启动装配入口
- `apps/src/components/layout/app-bootstrap.tsx`：界面初始化编排
- `apps/src/components/layout/app-bootstrap.tsx`：刷新流程与运行期协同
- `apps/src/app/settings/`：设置域门面，继续向子模块分发

### 3.2 桌面端壳层入口

- `apps/src-tauri/src/lib.rs`：Tauri 应用装配入口
- `apps/src-tauri/src/commands/settings/mod.rs`：桌面端设置桥接命令
- `apps/src-tauri/src/service_runtime.rs`：桌面内嵌 service 生命周期
- `apps/src-tauri/src/rpc_client/mod.rs`：桌面端 RPC 调用基础设施
...
- 별도의 `scripts/bump-version.ps1`은 현재 없습니다. 릴리스 전에 루트 `Cargo.toml`, `apps/package.json`, Tauri 설정 및 두 `Cargo.lock`의 버전을 릴리스 체크리스트로 확인합니다.
...
3. `crates/service/src/gateway/observability/http_bridge/response_helpers.rs` 兼容分支较多，回归风险高。
...
- 新页面或前端交互：优先落在 `apps/src/app/`、`apps/src/components/`、`apps/src/lib/`
