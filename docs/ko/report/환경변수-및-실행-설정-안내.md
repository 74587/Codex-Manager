...
- `CODEXMANAGER_UPSTREAM_STREAM_TIMEOUT_MS`
- `CODEXMANAGER_FRONT_PROXY_MAX_BODY_BYTES`: 프런트 프록시의 일반 요청 본문 제한(바이트)입니다. 기본값 `0`은 일반 제한을 사용하지 않지만, zstd 요청에는 별도의 압축 해제 안전 제한이 계속 적용됩니다.
- `CODEXMANAGER_FRONT_PROXY_ZSTD_MAX_BODY_BYTES`: zstd 요청의 압축 해제 후 본문 안전 제한(바이트)입니다. 기본값은 `268435456`(256 MiB)입니다. zstd 인코딩이 선언된 경우 압축 본문을 읽을 때도 같은 제한을 사용하며, 일반 제한이 `0`이 아니면 두 값 중 작은 값이 적용됩니다. 값은 `0`보다 커야 하며 `0` 또는 잘못된 값은 안전 기본값으로 돌아갑니다. 설정 > 환경 변수에서 저장하면 이후 요청부터 즉시 적용됩니다.
...
- `CODEXMANAGER_CODEX_IMAGE_MAIN_MODEL`: Images API 호환 엔드포인트 내부에서 사용하는 메인 대화 모델. 기본값 `gpt-6-luna`.
