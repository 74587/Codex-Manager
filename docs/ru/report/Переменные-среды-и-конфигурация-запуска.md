...
- `CODEXMANAGER_UPSTREAM_STREAM_TIMEOUT_MS`
- `CODEXMANAGER_FRONT_PROXY_MAX_BODY_BYTES`: общий лимит тела запроса front proxy в байтах. Значение по умолчанию `0` отключает общий лимит, но для zstd-запросов продолжает действовать отдельный безопасный лимит распаковки.
- `CODEXMANAGER_FRONT_PROXY_ZSTD_MAX_BODY_BYTES`: безопасный лимит тела zstd-запроса после распаковки в байтах. Значение по умолчанию `268435456` (256 MiB). Если zstd-кодирование указано явно, тот же лимит ограничивает чтение сжатого тела; если общий лимит не равен `0`, применяется меньшее из двух значений. Значение должно быть больше `0`; `0` или некорректное значение возвращает безопасный лимит по умолчанию. Сохранение в Settings > Environment variables применяется к последующим запросам без перезапуска.
...
- `CODEXMANAGER_CODEX_IMAGE_MAIN_MODEL`: основная conversation-модель, используемая внутри Images API compatibility endpoints. Значение по умолчанию `gpt-6-luna`.
