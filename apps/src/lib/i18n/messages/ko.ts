...
export const KO_MESSAGES: MessageCatalog = {
  ...
  "主对话模型，推荐直接使用 gpt-6-sol 作为默认工作模型":
    "주 대화 모델입니다. 기본 작업 모델로 gpt-6-sol 사용을 권장합니다.",
  ...
  "本机 Codex 的账号直连请求不会写入此日志":
    "이 컴퓨터의 Codex 계정 직결 요청은 이 로그에 기록되지 않습니다",
  "下方仍展示 CodexManager 已记录的网关请求，包括其他客户端通过平台密钥产生的流量。":
    "아래에는 플랫폼 키를 사용하는 다른 클라이언트의 트래픽을 포함하여 CodexManager가 기록한 게이트웨이 요청이 계속 표시됩니다.",
  去切换为本地网关: "로컬 게이트웨이로 전환",
  仅网关流量: "게이트웨이 트래픽만",
  "当前筛选下暂无已记录的网关请求；本机 Codex 的账号直连请求不会写入此日志。":
    "현재 필터와 일치하는 기록된 게이트웨이 요청이 없습니다. 이 컴퓨터의 Codex 계정 직결 요청은 이 로그에 기록되지 않습니다.",
  未分配: "미할당",
