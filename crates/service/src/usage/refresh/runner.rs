...
use super::{
    ...
    WARMUP_CRON_ENABLED, WARMUP_CRON_EXPRESSION,
};
...
/// # 返回
/// 无
pub(super) async fn usage_polling_loop() {
...
/// # 返回
/// 无
pub(super) async fn gateway_keepalive_loop() {
    run_dynamic_poll_loop(
        "gateway keepalive",
        ...
        || async { run_gateway_keepalive_once() },
...
/// # 返回
/// 无
pub(super) async fn token_refresh_polling_loop() {
    run_dynamic_poll_loop(
...
pub(super) async fn warmup_cron_loop() {
    ...
    'scheduler: loop {
        ...
        if !WARMUP_CRON_ENABLED.load(Ordering::Relaxed) {
            signal_version = wait_for_warmup_cron_change(signal_version, None).await.0;
            continue;
        }
        ...
        let next_run_at = match next_cron_after(expression.as_str(), Local::now()) {
            ...
            Err(err) => {
                ...
                signal_version =
                    wait_for_warmup_cron_change(signal_version, Some(Duration::from_secs(60)))
                        .await
                        .0;
                continue;
            }
        ...
        loop {
            ...
            let (next_signal_version, _) =
                wait_for_warmup_cron_change(signal_version, Some(delay)).await;
        ...
        if let Err(error) = crate::account_warmup::schedule_cron_warmup() {
            log::warn!("account warmup cron scheduling error: {error}");
        }
    }
}
...
async fn wait_for_warmup_cron_change(last_seen: u64, timeout: Option<Duration>) -> (u64, bool) {
    let effective_timeout = timeout
        .unwrap_or(Duration::from_millis(500))
        .min(Duration::from_millis(500));
    let current_version = warmup_cron_signal_version();
    if current_version != last_seen {
        return (current_version, false);
    }
    tokio::time::sleep(effective_timeout).await;
    let version = warmup_cron_signal_version();
    (version, version == last_seen)
}
...
/// # 返回
/// 无
async fn run_dynamic_poll_loop<F, Fut, L, E, I, J, B>(
    loop_name: &str,
    enabled: E,
    interval_secs: I,
    jitter_secs: J,
    ...
    B: Fn(u64) -> u64,
{
    ...
    loop {
        ...
        let sampled_jitter = if jitter_cap_secs == 0 {
            ...
            Duration::from_secs(rand::thread_rng().gen_range(0..=jitter_cap_secs))
        };
